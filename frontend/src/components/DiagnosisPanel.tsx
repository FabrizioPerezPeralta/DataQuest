...
const nfColorMap: Record<string, string> = {
  '1NF': 'bg-orange-100 text-orange-700 border-orange-200',
  '2NF': 'bg-amber-100 text-amber-700 border-amber-200',
  '3FN': 'bg-indigo-100 text-indigo-700 border-indigo-200',
  'BCNF': 'bg-emerald-100 text-emerald-700 border-emerald-200',
};
...
const Tooltip: React.FC<{ text: string; children: React.ReactNode }> = ({ text, children }) => {
  ...
  return (
    <span className="relative inline-block" onMouseEnter={() => setVisible(true)} onMouseLeave={() => setVisible(false)}>
      {children}
      {visible && (
        <span className="absolute z-50 bottom-full left-1/2 -translate-x-1/2 mb-2 w-56 bg-slate-900 text-slate-100 text-[11px] rounded-lg px-3 py-2 shadow-xl pointer-events-none animate-fade-in leading-relaxed text-center">
          {text}
...
const StepCard: React.FC<{ step: { step: string; explanation: string; violation_detail: string; rule_codd: string }; idx: number }> = ({ step, idx }) => {
  ...
  return (
    <div className="rounded-xl border border-amber-200 overflow-hidden animate-fade-in" style={{ animationDelay: `${idx * 100}ms` }}>
      ...
      {expanded && (
        <div className="p-4 space-y-4 bg-white border-t border-amber-100 animate-fade-in">
          {/* Explanation */}
...
export const DiagnosisPanel: React.FC<DiagnosisPanelProps> = ({ diagnosis }) => {
  ...
  const nfClass = nfColorMap[diagnosis.current_nf] || 'bg-slate-100 text-slate-700 border-slate-200';
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
      ...
      <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
        ...
        <span className={`px-3 py-1 rounded-full border text-xs font-bold ${nfClass}`}>
        ...
        {diagnosis.suggestions?.length > 0 && (
          <div className="p-4 bg-emerald-50 rounded-xl border border-emerald-200">
            <p className="text-xs font-semibold text-emerald-700 uppercase tracking-wider flex items-center gap-1.5 mb-3">
              <Lightbulb className="w-3.5 h-3.5" /> Sugerencias para Normalizar
            </p>
